...
ConVar lantern_minforce( "lantern_minforce", "700", FCVAR_REPLICATED | FCVAR_CHEAT );
ConVar lantern_maxforce( "lantern_maxforce", "1500", FCVAR_REPLICATED | FCVAR_CHEAT );
ConVar lantern_maxmass( "lantern_maxmass", "250", FCVAR_REPLICATED | FCVAR_CHEAT );
ConVar lantern_tracelength( "lantern_tracelength", "250", FCVAR_REPLICATED | FCVAR_CHEAT );
ConVar lantern_chargetime("lantern_chargetime", "2", FCVAR_REPLICATED | FCVAR_CHEAT );
ConVar lantern_pullforce( "lantern_pullforce", "4000", FCVAR_REPLICATED | FCVAR_CHEAT );
ConVar lantern_cone( "lantern_cone", "0.97", FCVAR_REPLICATED | FCVAR_CHEAT );
ConVar lantern_ball_cone( "lantern_ball_cone", "0.997", FCVAR_REPLICATED | FCVAR_CHEAT );
ConVar player_throwforce2( "player_throwforce2", "1000", FCVAR_REPLICATED | FCVAR_CHEAT );
...
#define PHYSCANNON_BLAST_SPRITE "sprites/orangecore2"
	...
	CLIENTEFFECT_REGISTER_BEGIN( PrecacheEffectLantern )
...
class CTraceFilterNoOwnerTestLantern : public CTraceFilterSimple
{
public:
	DECLARE_CLASS( CTraceFilterNoOwnerTestLantern, CTraceFilterSimple );
	
	CTraceFilterNoOwnerTestLantern( const IHandleEntity *passentity, int collisionGroup )
...
struct game_shadowcontrol_paramsLantern_t : public hlshadowcontrol_params_t
{
	DECLARE_SIMPLE_DATADESC();
};

BEGIN_SIMPLE_DATADESC( game_shadowcontrol_paramsLantern_t )
...
//-----------------------------------------------------------------------------
class CGrabControllerLantern : public IMotionEvent
{
public:

	CGrabControllerLantern( void );
	~CGrabControllerLantern( void );
...
private:
	...
	game_shadowcontrol_paramsLantern_t	m_shadow;
...
void CGrabControllerLantern::OnRestore()
{
	if ( m_controller )
	{
...
void CGrabControllerLantern::SetTargetPosition( const Vector &target, const QAngle &targetOrientation )
{
...
// Output : float
//-----------------------------------------------------------------------------
float CGrabControllerLantern::ComputeError()
{
	if ( m_errorTime <= 0 )
		return 0;
...
void CGrabControllerLantern::ComputeMaxSpeed( CBaseEntity *pEntity, IPhysicsObject *pPhysics )
{
#ifndef CLIENT_DLL
	m_shadow.maxSpeed = 1000;
	m_shadow.maxAngular = DEFAULT_MAX_ANGULAR;
	...
	float flMaxMass = lantern_maxmass.GetFloat();
	if ( flMass <= flMaxMass )
		return;
...
QAngle CGrabControllerLantern::TransformAnglesToPlayerSpace( const QAngle &anglesIn, CBasePlayer *pPlayer )
{
...
QAngle CGrabControllerLantern::TransformAnglesFromPlayerSpace( const QAngle &anglesIn, CBasePlayer *pPlayer )
{
	if ( m_bIgnoreRelativePitch )
	{
		matrix3x4_t test;
...
void CGrabControllerLantern::AttachEntity( CBasePlayer *pPlayer, CBaseEntity *pEntity, IPhysicsObject *pPhys, bool bIsMegaPhysCannon, const Vector &vGrabPosition, bool bUseGrabPosition )
{
...
void CGrabControllerLantern::DetachEntity( bool bClearVelocity )
{
...
IMotionEvent::simresult_e CGrabControllerLantern::Simulate( IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime, Vector &linear, AngularImpulse &angular )
{
	game_shadowcontrol_paramsLantern_t shadowParams = m_shadow;
...
float CGrabControllerLantern::GetSavedMass( IPhysicsObject *pObject )
{
...
class CPlayerPickupControllerLantern : public CBaseEntity
{
	DECLARE_CLASS( CPlayerPickupControllerLantern, CBaseEntity );
public:
	...
	CGrabControllerLantern &GetGrabController() { return m_grabController; }

private:
	CGrabControllerLantern		m_grabController;
	CBasePlayer			*m_pPlayer;
};

LINK_ENTITY_TO_CLASS( player_pickup, CPlayerPickupControllerLantern );
...
//			*pObject - 
//-----------------------------------------------------------------------------
void CPlayerPickupControllerLantern::Init( CBasePlayer *pPlayer, CBaseEntity *pObject )
{
#ifndef CLIENT_DLL
...
// Input  : bool - 
//-----------------------------------------------------------------------------
void CPlayerPickupControllerLantern::Shutdown( bool bThrown )
{
#ifndef CLIENT_DLL
...
void CPlayerPickupControllerLantern::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	if ( ToBasePlayer(pActivator) == m_pPlayer )
	{
		...
		if ( m_pPlayer->m_nButtons & IN_ATTACK )
		{
			...
			vecLaunch *= player_throwforce2.GetFloat() * massFactor;
...
//-----------------------------------------------------------------------------
bool CPlayerPickupControllerLantern::IsHoldingEntity( CBaseEntity *pEnt )
{
...
/*void PlayerPickupObject( CBasePlayer *pPlayer, CBaseEntity *pObject )
{
	...
	CPlayerPickupControllerLantern *pController = (CPlayerPickupControllerLantern *)CBaseEntity::Create( "player_pickup", pObject->GetAbsOrigin(), vec3_angle, pPlayer );
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------
//  CPhysCannonEffectLantern class
//----------------------------------------------------------------------------------------------------------------------------------------------------------

class CPhysCannonEffectLantern
{
public:
	CPhysCannonEffectLantern( void ) : m_vecColor( 255, 255, 255 ), m_bVisible( true ), m_nAttachment( -1 ) {};
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------
//  CPhysCannonEffectLanternBeamLantern class
//----------------------------------------------------------------------------------------------------------------------------------------------------------

class CPhysCannonEffectLanternBeamLantern
{
public:
	CPhysCannonEffectLanternBeamLantern( void ) : m_pBeam( NULL ) {};

	~CPhysCannonEffectLanternBeamLantern( void )
	{
		Release();
	}
...
{
public:
	DECLARE_CLASS( CWeaponLantern, CBaseHL2MPCombatWeapon );
...
#define	NUM_GLOW_SPRITES ((CWeaponLantern::PHYSCANNON_GLOW6-CWeaponLantern::PHYSCANNON_GLOW1)+1)
#define NUM_ENDCAP_SPRITES ((CWeaponLantern::PHYSCANNON_ENDCAP3-CWeaponLantern::PHYSCANNON_ENDCAP1)+1)
...
#define	NUM_PHYSCANNON_BEAMS	3
	...
	CPhysCannonEffectLantern		m_Parameters[NUM_PHYSCANNON_PARAMETERS];	// Interpolated parameters for the effects
	CPhysCannonEffectLanternBeamLantern	m_Beams[NUM_PHYSCANNON_BEAMS];				// Beams
...
private:
	CWeaponLantern( const CWeaponLantern & );
...
// Purpose: Precache
//-----------------------------------------------------------------------------
void CWeaponLantern::Precache( void )
{
...
// On Remove
//-----------------------------------------------------------------------------
void CWeaponLantern::UpdateOnRemove(void)
{
	DestroyEffects( );
...
#ifdef CLIENT_DLL
void CWeaponLantern::OnDataChanged( DataUpdateType_t type )
{
...
// Sprite scale factor 
//-----------------------------------------------------------------------------
inline float CWeaponLantern::SpriteScaleFactor() 
{
	return 1.0f;
}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponLantern::Deploy( void )
{
	CloseElements();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponLantern::SetViewModel( void )
{
...
//-----------------------------------------------------------------------------
void CWeaponLantern::ForceDrop( void )
{
	CloseElements();
	DetachObject();
	StopEffects();
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponLantern::DropIfEntityHeld( CBaseEntity *pTarget )
{
	if ( pTarget == NULL )
		return false;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponLantern::Drop( const Vector &vecVelocity )
{
	ForceDrop();
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CWeaponLantern::CanHolster( void ) 
{ 
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponLantern::Holster( CBaseCombatWeapon *pSwitchingTo )
{
	//Don't holster this weapon if we're holding onto something
	if ( m_bActive )
		return false;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponLantern::PrimaryFireEffect( void )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
// Punt non-physics
//-----------------------------------------------------------------------------
void CWeaponLantern::PuntNonVPhysics( CBaseEntity *pEntity, const Vector &forward, trace_t &tr )
{
...
// What happens when the physgun picks up something 
//-----------------------------------------------------------------------------
void CWeaponLantern::Physgun_OnPhysGunPickup( CBaseEntity *pEntity, CBasePlayer *pOwner, PhysGunPickup_t reason )
{
	// If the target is debris, convert it to non-debris
...
// Punt vphysics
//-----------------------------------------------------------------------------
void CWeaponLantern::PuntVPhysics( CBaseEntity *pEntity, const Vector &vecForward, trace_t &tr )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
// Input  : - 
//-----------------------------------------------------------------------------
void CWeaponLantern::ApplyVelocityBasedForce( CBaseEntity *pEntity, const Vector &forward )
{
#ifndef CLIENT_DLL
	...
	{
		mass = MIN(mass, 1000);
		float flForceMin = lantern_minforce.GetFloat();
...
// Trace length
//-----------------------------------------------------------------------------
float CWeaponLantern::TraceLength()
{
	return lantern_tracelength.GetFloat();
}
...
//-----------------------------------------------------------------------------
void CWeaponLantern::PrimaryAttack( void )
{
	...
	/*if( m_bActive )
	{
		...
		if ( pHeld != NULL )
		{
			...
			if ( heldDist > lantern_tracelength.GetFloat() )
			{
	...
	float flPuntDistance = lantern_tracelength.GetFloat();
	VectorMA( start, flPuntDistance, forward, end );

	CTraceFilterNoOwnerTestLantern filter( pOwner, COLLISION_GROUP_NONE );
	trace_t tr;
...
//-----------------------------------------------------------------------------
void CWeaponLantern::SecondaryAttack( void )
{
	return;
}	
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponLantern::WeaponIdle( void )
{
...
// Input  : *pObject - 
//-----------------------------------------------------------------------------
bool CWeaponLantern::AttachObject( CBaseEntity *pObject, const Vector &vPosition )
{
...
CWeaponLantern::FindObjectResult_t CWeaponLantern::FindObject( void )
{
	...
	trace_t tr;
	CTraceFilterNoOwnerTestLantern filter( pPlayer, COLLISION_GROUP_NONE );
	UTIL_TraceLine( start, end, MASK_SHOT|CONTENTS_GRATE, &filter, &tr );
	...
	if (!bAttach && !bPull)
	{
		pConeEntity = FindObjectInCone( start, forward, lantern_cone.GetFloat() );
	}
	...
	pullDir *= lantern_pullforce.GetFloat();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CBaseEntity *CWeaponLantern::FindObjectInCone( const Vector &vecOrigin, const Vector &vecDir, float flCone )
{
	...
	for( int i = 0 ; i < count ; i++ )
	{
		...
		trace_t tr;
		CTraceFilterNoOwnerTestLantern filter( GetOwner(), COLLISION_GROUP_NONE );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CGrabControllerLantern::UpdateObject( CBasePlayer *pPlayer, float flError )
{
	CBaseEntity *pEntity = GetAttached();
	if ( !pEntity )
		return false;
...
void CWeaponLantern::UpdateObject( void )
{
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
	Assert( pPlayer );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWeaponLantern::DetachObject( bool playSound, bool wasLaunched )
{
#ifndef CLIENT_DLL
	if ( m_bActive == false )
		return;
...
#ifdef CLIENT_DLL
void CWeaponLantern::ManagePredictedObject( void )
{
...
//-----------------------------------------------------------------------------
void CWeaponLantern::UpdateElementPosition( void )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
void CWeaponLantern::ClientThink( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWeaponLantern::ItemPreFrame()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponLantern::CheckForTarget( void )
{
#ifndef CLIENT_DLL
...
//-----------------------------------------------------------------------------
void CWeaponLantern::DoEffectIdle( void )
{
#ifdef CLIENT_DLL
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWeaponLantern::ItemPostFrame()
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
	if ( pOwner == NULL )
	{
	...
	// light it off.
	else if(m_bActiveLight && m_flNextPrimaryAttack < gpGlobals->curtime)
	{    
...
void CWeaponLantern::LaunchObject( const Vector &vecDir, float flForce )
{
	CBaseEntity *pObject = m_grabController.GetAttached();
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponLantern::CanPickupObject( CBaseEntity *pTarget )
{
#ifndef CLIENT_DLL
	if ( pTarget == NULL )
		return false;
	...
	return CBasePlayer::CanPickupObject( pTarget, lantern_maxmass.GetFloat(), 0 );
#else
	return false;
#endif
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponLantern::OpenElements( void )
{
	if ( m_bOpen )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponLantern::CloseElements( void )
{
	if ( m_bOpen == false )
		return;
...
// Output : float
//-----------------------------------------------------------------------------
float CWeaponLantern::GetLoadPercentage( void )
{
	float loadWeight = m_grabController.GetLoadWeight();
	loadWeight /= lantern_maxmass.GetFloat();	
...
// Output : CSoundPatch
//-----------------------------------------------------------------------------
CSoundPatch *CWeaponLantern::GetMotorSound( void )
{
	if ( m_sndMotor == NULL )
	{
...
// Shuts down sounds
//-----------------------------------------------------------------------------
void CWeaponLantern::StopLoopingSounds()
{
	if ( m_sndMotor != NULL )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponLantern::DestroyEffects( void )
{
#ifdef CLIENT_DLL
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponLantern::StopEffects( bool stopSound )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponLantern::StartEffects( void )
{
#ifdef CLIENT_DLL
...
//-----------------------------------------------------------------------------
void CWeaponLantern::DoEffectClosed( void )
{
...
//-----------------------------------------------------------------------------
void CWeaponLantern::DoEffectReady( void )
{
...
// Holding effects
//-----------------------------------------------------------------------------
void CWeaponLantern::DoEffectHolding( void )
{
...
// Launch effects
//-----------------------------------------------------------------------------
void CWeaponLantern::DoEffectLaunch( Vector *pos )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
	if ( pOwner == NULL )
		return;
...
//-----------------------------------------------------------------------------
void CWeaponLantern::DoEffectNone( void )
{
#ifdef CLIENT_DLL
...
// Input  : effectType - 
//-----------------------------------------------------------------------------
void CWeaponLantern::DoEffect( int effectType, Vector *pos )
{
...
// Output : const char
//-----------------------------------------------------------------------------
const char *CWeaponLantern::GetShootSound( int iIndex ) const
{
...
//			effect parameter
//-----------------------------------------------------------------------------
void CWeaponLantern::GetEffectParameters( EffectType_t effectID, color32 &color, float &scale, IMaterial **pMaterial, Vector &vecAttachment )
{
...
//-----------------------------------------------------------------------------
bool CWeaponLantern::IsEffectVisible( EffectType_t effectID )
{
...
//-----------------------------------------------------------------------------
void CWeaponLantern::DrawEffectSprite( EffectType_t effectID )
{
	color32 color;
	float scale;
	IMaterial *pMaterial;
...
//-----------------------------------------------------------------------------
void CWeaponLantern::DrawEffects( void )
{
...
//-----------------------------------------------------------------------------
int CWeaponLantern::DrawModel( int flags )
{
...
//-----------------------------------------------------------------------------
void CWeaponLantern::ViewModelDrawn( C_BaseViewModel *pBaseViewModel )
{
	// Render our effects
	DrawEffects();
...
//-----------------------------------------------------------------------------
bool CWeaponLantern::IsTransparent( void )
{
	return true;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponLantern::NotifyShouldTransmit( ShouldTransmitState_t state )
{
...
{
	CWeaponLantern *pCannon = dynamic_cast<CWeaponLantern *>(pActiveWeapon);
	if ( pCannon )
	{
...
{
	CPlayerPickupControllerLantern *pController = dynamic_cast<CPlayerPickupControllerLantern *>(pPickupControllerEntity);
...
float PhysCannonGetHeldObjectMass( CBaseCombatWeapon *pActiveWeapon, IPhysicsObject *pHeldObject )
{
	...
	if ( pCannon )
	{
		CGrabControllerLantern &grab = pCannon->GetGrabController();
...
float PlayerPickupGetHeldObjectMass( CBaseEntity *pPickupControllerEntity, IPhysicsObject *pHeldObject )
{
	...
	if ( pController )
	{
		CGrabControllerLantern &grab = pController->GetGrabController();
		mass = grab.GetSavedMass( pHeldObject );
	}
	return mass;
}
